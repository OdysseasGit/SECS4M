"""
Script to feed the ERA5 data to SECS
...
weather_data_dir = '/path/to/ERA5/'
rnn_dir = '/path/to/parentRNNdir'

predictions_dir = os.path.join(os.sep, rnn_dir, 'PREDICTIONS_ERA')
os.makedirs(predictions_dir, exist_ok=True)

for year in years:
    yearrange = str(year) + '0101-' + str(year) + '1231'

    predictions_with_era_flpth = os.path.join(os.sep, predictions_dir, str(year) + '_TWSO_predicted_withERA5_v2.nc')

    era_tasmin_flpth = os.path.join(os.sep, weather_data_dir,
                                    'tasmin_day_reanalysis_era5_r1i1p1_' + yearrange + '_prepr.nc')
    era_tasmax_flpth = os.path.join(os.sep, weather_data_dir,
                                    'tasmax_day_reanalysis_era5_r1i1p1_' + yearrange + '_prepr.nc')
    era_pr_flpth = os.path.join(os.sep, weather_data_dir, 'pr_day_reanalysis_era5_r1i1p1_' + yearrange + '_prepr.nc')

    if not os.path.exists(predictions_with_era_flpth):
        print('Working on predictions with ERA5: ', predictions_with_era_flpth)

        era_tasmin = xr.open_dataset(era_tasmin_flpth)
        era_tasmax = xr.open_dataset(era_tasmax_flpth)
        era_pr = xr.open_dataset(era_pr_flpth)
        ...
        lsm_binary_remaped_flpth = os.path.join(os.sep, '/path/to/lsm_binary_remaped_era.nc')
        cdo = cdo.Cdo()

        if not os.path.exists(lsm_binary_remaped_flpth):
            cdo.remapnn(era_tasmin_flpth, input=lsm_binary_flpth, output=lsm_binary_remaped_flpth)

        ''' END OF CONSTRUCT THE BINARY LAND SEA MASK '''

        ''' Construct the full weather dataset '''
        era_full = xr.merge([era_tasmax, era_tasmin, era_pr])
        era_full = era_full.drop_dims('bnds')

        era_full['tasmax'] = era_full['tasmax'] - 273.16
        era_full['tasmin'] = era_full['tasmin'] - 273.16
        era_full['pr'] = era_full['pr'] * 24 * 3600 / 10

        era_full['pr'] = era_full['pr'].cumsum(dim='time')  # make it rain_total
        ...
        era_full_lagged = add_lags(era_full)
        ...
        era_full_lagged = add_lags(era_full, lag_days=5)
        verify_lags(era_full, era_full_lagged, day_index=1, lag_days=5)

        ''' Cut the dates to the proper range: len(time)-240: len(time) '''
        time = era_full_lagged['time']
        ...
        era_full_lagged_240d = era_full_lagged.sel(time=slice(start_date, end_date))

        print(era_full_lagged_240d)

        ''' Calculate the DOY for each time point and create a DataArray '''
        doy = pd.to_datetime(era_full_lagged_240d['time'].values).dayofyear
        doy_1d = xr.DataArray(doy, coords=[era_full_lagged_240d['time']], dims=['time'], name='doy')

        # Now expand the DOY DataArray to include lat and lon dimensions
        # Use the broadcasting mechanism within xarray
        doy_broadcasted = doy_1d.broadcast_like(era_full_lagged_240d)
        ...
        era_full_lagged_240d['doy'] = doy_broadcasted
        ...
        era_full_lagged_240d_reordered = era_full_lagged_240d[var_order]

        ''' Convert to numpy array '''
        # Convert the Dataset to a dictionary of numpy arrays
        dict_of_arrays = {}
        for var in era_full_lagged_240d_reordered.data_vars:
            # print(f"Processing variable: {var}")  # Print the variable name for checking order
            dict_of_arrays[var] = era_full_lagged_240d_reordered[var].values
        # stack the variables along a new axis
        np_reordered = np.stack([dict_of_arrays[var] for var in era_full_lagged_240d_reordered.data_vars], axis=-1)
        ...
        pred_xr = xr.DataArray(
            ...
            dims=['time', 'lat', 'lon'],
            coords={
                'time': era_full_lagged_240d.coords['time'],
                'lat': era_full_lagged_240d.coords['lat'],
                'lon': era_full_lagged_240d.coords['lon']
            },
        ...
        pred_xr_masked.to_netcdf(predictions_with_era_flpth)
        print('Finished predictions with ERA5: ', predictions_with_era_flpth)

    else:
        print('Found existing predictions with ERA5: ', predictions_with_era_flpth)
